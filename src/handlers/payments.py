...
from db import get_user_by_telegram_id, record_successful_payment
...
def get_db_user_id(telegram_user_id):
...
async def handle_buy_plan(callback: types.CallbackQuery):
    ...
    user_id = get_db_user_id(callback.from_user.id)
    payload = build_invoice_payload(plan_code, callback.from_user.id)
    provider_data = build_provider_data(plan)
    log_event(logger, logging.INFO, "Invoice requested", user_id=user_id, plan=plan_code, amount=plan["price_minor_units"])
    ...
    try:
        ...
        log_event(logger, logging.INFO, "Invoice sent", user_id=user_id, plan=plan_code, amount=plan["price_minor_units"])
    except Exception as exc:
        log_exception(logger, "Invoice send failed", exc, user_id=user_id, plan=plan_code, amount=plan["price_minor_units"])
...
async def handle_pre_checkout_query(pre_checkout_query: types.PreCheckoutQuery):
    user_id = get_db_user_id(pre_checkout_query.from_user.id)
    log_event(logger, logging.INFO, "Pre-checkout received", user_id=user_id, amount=pre_checkout_query.total_amount)
    try:
        plan_code, telegram_user_id = parse_invoice_payload(pre_checkout_query.invoice_payload)
    except (TypeError, ValueError):
        log_event(logger, logging.WARNING, "Pre-checkout rejected", user_id=user_id, reason="invalid_payload", amount=pre_checkout_query.total_amount)
    ...
    if plan_code not in PAYMENT_PLANS or telegram_user_id != pre_checkout_query.from_user.id:
        log_event(logger, logging.WARNING, "Pre-checkout rejected", user_id=user_id, plan=plan_code, reason="stale_or_mismatched_invoice", amount=pre_checkout_query.total_amount)
        await pre_checkout_query.answer(
            ok=False,
    ...
    log_event(logger, logging.INFO, "Pre-checkout approved", user_id=user_id, plan=plan_code, amount=pre_checkout_query.total_amount)
...
async def handle_successful_payment(message: types.Message, state: FSMContext):
    ...
    user_id = get_db_user_id(message.from_user.id)

    try:
        plan_code, telegram_user_id = parse_invoice_payload(payment.invoice_payload)
    except (TypeError, ValueError):
        log_event(logger, logging.WARNING, "Payment received with invalid payload", user_id=user_id, amount=payment.total_amount, tg_charge=payment.telegram_payment_charge_id)
    ...
    if telegram_user_id != message.from_user.id:
        log_event(logger, logging.WARNING, "Payment received for mismatched user", user_id=user_id, plan=plan_code, amount=payment.total_amount, tg_charge=payment.telegram_payment_charge_id)
        await message.answer("Платёж получен, но принадлежит другому пользователю.")
        return

    plan = PAYMENT_PLANS.get(plan_code)
    if not plan:
        log_event(logger, logging.WARNING, "Payment received for unknown plan", user_id=user_id, plan=plan_code, amount=payment.total_amount, tg_charge=payment.telegram_payment_charge_id)
        await message.answer("Платёж получен, но тариф не найден. Напишите в поддержку.")
        return

    log_event(logger, logging.INFO, "Payment received", user_id=user_id, plan=plan_code, amount=payment.total_amount, tg_charge=payment.telegram_payment_charge_id)
    ...
    try:
        ...
        log_exception(logger, "Payment save failed", exc, user_id=user_id, plan=plan_code, amount=payment.total_amount, tg_charge=payment.telegram_payment_charge_id)
        await message.answer("Оплата получена, но доступ пока не открылся автоматически. Напишите в поддержку.")
        return

    log_event(logger, logging.INFO, "Payment saved", user_id=user_id, plan=plan_code, amount=payment.total_amount, tg_charge=payment.telegram_payment_charge_id)
